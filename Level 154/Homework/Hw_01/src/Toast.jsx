...
function Toast({ content, expiresIn, colors, funct }) {
  ...
  return (
    <div className={`w-[50%] min-w-[250px] max-w-[400px] h-[200px] rounded border px-10 py-5 relative flex justify-between flex-col`} style={{ backgroundColor: colors.background, color: colors.color }}>
      ...
      <h1 className={`text-center text-[30px] font-[700] ${time <= 3000 ? 'text-red-600' : 'text-green-600'}`}>{String(time).slice(0, (String(time).length - 3))}</h1>
